...
const callableDefinitionRegex =
  /^\s*(?:(forward|native|public|static|stock)\s+)?([A-Za-z_@][\w@:]*)\s*\(([^)]*)\)/;
const callableStartRegex =
  /^\s*(?:(?:forward|native|public|static|stock)\s+)?[A-Za-z_@][\w@:]*\s*\(/;
...
const localVarDeclRegex =
  /\b(?:new|static|const|stock)\b(?:\s+(?:new|static|const|stock)\b)*/;
...
function joinMultiLineSignature(
  lines: string[],
  startIndex: number,
  ...
  for (
    let i = startIndex + 1;
    i < lines.length && i <= startIndex + maxLookahead;
    i++
  ) {
...
function findIdentifierAtCursor(
  content: string,
  cursorIndex: number,
): { identifier: string; isCallable: boolean; isTag: boolean } {
  ...
  while (
    checkParen < content.length &&
    StringHelpers.isWhitespace(content[checkParen])
  )
    checkParen++;
  if (checkParen < content.length && content[checkParen] === "(")
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    if (inBlockComment) {
      if (trimmedLine.includes("*/")) {
        ...
        const commentPart = originalLine
          .substring(0, endIdx)
          .replace(/^\s*\*/, "")
          .trim();
    ...
    if (bracketDepth > 0) {
      ...
      if (currentFunctionStartLine >= 0) {
        ...
        if (isDeclStart || (currentVarDecl && !currentVarDecl.isGlobal)) {
          ...
          if (isDeclStart && declMatch) {
            declPart = lineContent.substring(
          ...
          if (!/^\s*(?:new|static|const|stock|\s)+$/.test(lineContent)) {
            ...
            for (const seg of segments) {
              ...
              const identMatch = trimmedSeg.match(
                /^(?:([A-Za-z_@][\w@]*):)?([A-Za-z_@][\w@]*)/,
              );
              if (identMatch) {
                ...
                if (!pawnKeywords.includes(varName)) {
                  ...
                  if (varCol >= 0) {
                    ...
                    const varCleanLabel =
                      prefix.join(" ") + " " + trimmedSeg.split("=")[0].trim();
                    currentFunctionLocals.push({
                      identifier: varName,
                      line: lineIndex,
                      col: varCol,
                      len: varName.length,
                      isConst: currentVarDecl!.isConst,
                      label: varCleanLabel,
                    });
                    ...
                    results.semanticTokens.push({
                      ...
                      tokenType: 2,
                      tokenModifiers: modifiers, // variable
                    });
                  }
                }
              }
          ...
          if (
            ...
            (!lineContent.endsWith(",") &&
              !lineContent.endsWith("\\") &&
              !/^\s*(?:new|static|const|stock|\s)+$/.test(lineContent))
          ) {
            currentVarDecl = null;
          }
        }
      ...
      if (bracketDepth === 0 && currentFunctionStartLine >= 0) {
        for (const local of currentFunctionLocals) {
          results.localVariables.push({
            ...
            file: fileUri,
            range: {
              start: { line: local.line, character: local.col },
              end: { line: local.line, character: local.col + local.len },
            },
    ...
    if (
      trimmedContent.startsWith("#include") ||
      trimmedContent.startsWith("#tryinclude")
    ) {
      ...
      if (match?.[1]) {
        results.headerInclusions.push({
          ...
          isSilent: trimmedContent.startsWith("#tryinclude"),
          start: { line: lineIndex, character: 0 },
          end: { line: lineIndex, character: originalLine.length },
        });

        // Semantic: #include keyword
        const keyword = trimmedContent.startsWith("#tryinclude")
          ? "#tryinclude"
          : "#include";
      ...
      if (match) {
        ...
        if (params !== undefined) {
          ...
          results.callables.push({
            ...
            start: {
              line: lineIndex,
              character: originalLine.indexOf(identifier, searchPos),
            },
            end: {
              line: lineIndex,
              character:
                originalLine.indexOf(identifier, searchPos) + identifier.length,
            },
            parameters: params
              ? params.split(",").map((p) => ({ label: p.trim() }))
              : [],
            documentation: `Macro: ${lineContent}`,
            isForward: false,
          });
          ...
          if (macroCol >= 0) {
            results.semanticTokens.push({
              ...
              tokenType: 1,
              tokenModifiers: 1, // macro, readonly
            });
          }
        } else {
          const searchPos = originalLine.indexOf("#define") + 7;
          results.constants.push({
            identifier,
            value: value.trim(),
            label: `#define ${identifier} ${value.trim()}`,
            file: fileUri,
            range: {
              start: { line: lineIndex, character: 0 },
              end: { line: lineIndex, character: originalLine.length },
      ...
      results.semanticTokens.push({
        line: lineIndex,
        char: originalLine.indexOf("enum"),
        length: 4,
        tokenType: 5,
        tokenModifiers: 0, // keyword
      });

      const enumNameMatch = lineContent.match(
        /^enum\s+(?:([A-Za-z_@][\w@:]*)\s*)?{?|enum\s+\([^)]+\)\s*{?/,
      );
          ...
          if (valueMatch) {
            ...
            if (enumVal !== "}" && !pawnKeywords.includes(enumVal)) {
              results.constants.push({
                ...
                file: fileUri,
                range: {
                  start: { line: ei, character: 0 },
                  end: { line: ei, character: lines[ei].length },
                },
              });
              ...
              const enumCol = lines[ei].indexOf(
                enumVal,
                tagName ? lines[ei].indexOf(":") : 0,
              );
              if (enumCol >= 0) {
                results.semanticTokens.push({
                  line: ei,
                  char: enumCol,
                  length: enumVal.length,
                  tokenType: 3,
                  tokenModifiers: 1, // enumMember, readonly
                });
              }
            }
          }
      ...
      if (
        callableStartRegex.test(lineContent) &&
        !callableDefinitionRegex.test(lineContent)
      ) {
      ...
      if (callableMatch && callableMatch[2] !== "enum") {
        ...
        if (specifier) {
          searchPos =
            originalLine.indexOf(specifier, searchPos) + specifier.length;
        }
        ...
        if (fnCol >= 0) {
          ...
          results.semanticTokens.push({
            ...
            length: identifier.length,
            tokenType: 0,
            tokenModifiers: modifiers, // function
          });
        }
          ...
          for (const paramStr of paramList) {
            ...
            for (let si = searchLineIdx; si < sigLines.length; si++) {
              ...
              if (col >= 0) {
                ...
                const after =
                  col + paramName.length < sigLines[si].text.length
                    ? sigLines[si].text[col + paramName.length]
                    : " ";
                if (
                  !StringHelpers.isAlphaNum(before) &&
                  !StringHelpers.isAlphaNum(after)
                ) {
                  results.semanticTokens.push({
                    line: sigLines[si].ln,
                    char: col,
          ...
          for (const paramStr of params.split(",")) {
            ...
            if (pName && !pawnKeywords.includes(pName)) {
              currentFunctionLocals.push({
                ...
                len: pName.length,
                isConst: false,
                label: `(param) ${paramStr.trim()}`,
              });
            }
          }
        }

        const existingCallableIndex = results.callables.findIndex(
        ...
        if (extraLinesConsumed > 0) {
          ...
          for (
            let skip = lineIndex + 1;
            skip <= lineIndex + extraLinesConsumed && skip < lines.length;
            skip++
          ) {
            const skipLine = stripComments(lines[skip]);
            const skipNoStrings = skipLine
              .replace(/"[^"]*"/g, "")
              .replace(/'[^']*'/g, "");
            bracketDepth +=
              (skipNoStrings.match(/{/g) || []).length -
              (skipNoStrings.match(/}/g) || []).length;
          }
        ...
        if (isDeclStart || (currentVarDecl && currentVarDecl.isGlobal)) {
          ...
          let declPart = lineContent;
          if (isDeclStart) {
            declPart = lineContent.replace(
              /^\s*(?:(?:new|static|const|public|stock)\s+)+/,
              "",
            );
          }
          ...
          if (!/^\s*(?:new|static|const|public|stock|\s)+$/.test(lineContent)) {
            ...
            let searchPos = originalLine.indexOf(lineContent);
            if (isDeclStart) {
              const modMatch = lineContent.match(
                /^\s*(?:(?:new|static|const|public|stock)\s+)+/,
              );
              if (modMatch) {
            ...
            for (const seg of segments) {
              ...
              if (identMatch) {
                ...
                const varName = identMatch[2];
                if (
                  !pawnKeywords.includes(varName) &&
                  !results.values.find((v) => v.identifier === varName)
                ) {
                  if (tagName) {
                    const tagCol = originalLine.indexOf(tagName, searchPos);
                    if (tagCol >= 0) {
                  ...
                  if (varCol >= 0) {
                    ...
                    results.values.push({
                      ...
                      file: fileUri,
                      range: {
                        start: { line: lineIndex, character: varCol },
                        end: {
                          line: lineIndex,
                          character: varCol + varName.length,
                        },
                      },
                      documentation: docComment.trim(),
                    });
          ...
          if (
            ...
            (!lineContent.endsWith(",") &&
              !lineContent.endsWith("\\") &&
              !/^\s*(?:new|static|const|public|stock|\s)+$/.test(lineContent))
          ) {
            currentVarDecl = null;
          }
        }
...
export function doDefinition(
  ...
  const localVar = localVars.find(
    (lv) =>
      lv.identifier === result.identifier &&
      position.line >= lv.scopeStartLine &&
      position.line <= lv.scopeEndLine,
  );
  if (localVar) {
  ...
  while ((match = stringRegex.exec(line)) !== null) {
    ...
    if (
      position.character >= match.index + 1 &&
      position.character <= match.index + 1 + stringContent.length
    ) {
      for (const taskFn of taskFunctions) {
        if (line.includes(`${taskFn}(`)) {
          const callable = symbols.callables.find(
            (clb) =>
              clb.identifier.toLowerCase() === stringContent.toLowerCase(),
          );
          if (callable)
            return VSCLS.Location.create(
              callable.file.toString(),
              VSCLS.Range.create(callable.start, callable.end),
            );
        }
      }
    }
  ...
  const value = symbols.values.find((val) =>
    potentialIdentifiers.some(
      (id) => id.toLowerCase() === val.identifier.toLowerCase(),
    ),
  );
  if (value) {
    if (
      data.uri === value.file.toString() &&
      position.line === value.range.start.line
    )
      return null;
  ...
  // 2. Check callables
  const callable = symbols.callables.find((clb) =>
    potentialIdentifiers.some(
      (id) => id.toLowerCase() === clb.identifier.toLowerCase(),
    ),
  );
  if (callable) {
    if (
      data.uri === callable.file.toString() &&
      position.line === callable.start.line
    )
      return null;
    return VSCLS.Location.create(
      callable.file.toString(),
  ...
  const constant = symbols.constants.find(
    (c) => c.identifier.toLowerCase() === identifierLower,
  );
  if (constant) {
    if (
      data.uri === constant.file.toString() &&
      position.line === constant.range.start.line
    )
      return null;
...
function findIdentifierBehindCursor(
  content: string,
  cursorIndex: number,
): string {
...
export function doCompletions(
  ...
  if (/^\s*#(?:try)?include\s*[<"]/.test(textBeforeCursor)) {
    ...
    const includePathMatch = textBeforeCursor.match(
      /#(?:try)?include\s*([<"])([^>"]*)$/,
    );
  ...
  const { values, callables, constants } = Helpers.getSymbols(
    data,
    dependenciesData,
  );
  ...
  constants.forEach((c) => {
    allItems.push({
      label: c.identifier,
      detail: c.label,
      kind: VSCLS.CompletionItemKind.Constant,
    });
  });
  ...
  values.forEach((v) => {
    allItems.push({
      ...
      insertText: v.identifier.startsWith("@")
        ? v.identifier.substring(1)
        : v.identifier,
  ...
  callables.forEach((c) => {
    allItems.push({
      ...
      insertText: c.identifier.startsWith("@")
...
function findFunctionIdentifier(
  content: string,
  cursorIndex: number,
  ...
  while (searchIndex >= 0) {
    ...
    if (char === ")") {
      ...
      if (parenDepth > 0) {
        ...
        while (
          endOfIdent > 0 &&
          StringHelpers.isWhitespace(content[endOfIdent - 1])
        )
          endOfIdent--;
        let startOfIdent = endOfIdent;
        while (
          startOfIdent > 0 &&
          StringHelpers.isAlphaNum(content[startOfIdent - 1])
        )
          startOfIdent--;
        ...
        const currentParamText = content.substring(
          currentParamStart,
          cursorIndex,
        );
...
export function doHover(
  ...
  if (result.identifier.startsWith("@"))
    idsToSearch.push(result.identifier.substring(1));
  ...
  if (result.isTag) {
    return {
      contents: [
        { language: "amxxpawn", value: `(tag) ${result.identifier}:` },
      ],
    };
  }

  // 1. Check variables (values) - high priority
  let value = symbols.values.find((v) =>
    idsToSearch.some((id) => id === v.identifier),
  );
  if (!value) {
    value = symbols.values.find((v) =>
      idsToSearch.some((id) => id.toLowerCase() === v.identifier.toLowerCase()),
    );
  }
  if (value) {
    ...
    return {
      contents: [
        { language: "amxxpawn", value: value.label },
  ...
  // 2. Check callables
  let callable = symbols.callables.find((c) =>
    idsToSearch.some((id) => id === c.identifier),
  );
  if (!callable) {
    callable = symbols.callables.find((c) =>
      idsToSearch.some((id) => id.toLowerCase() === c.identifier.toLowerCase()),
    );
  }
  if (callable) {
    return {
      contents: [
        { language: "amxxpawn", value: callable.label },
  ...
  // 3. Check constants
  let constant = symbols.constants.find((c) =>
    idsToSearch.some((id) => id === c.identifier),
  );
  if (!constant) {
    constant = symbols.constants.find((c) =>
      idsToSearch.some((id) => id.toLowerCase() === c.identifier.toLowerCase()),
    );
  }
  if (constant)
    return { contents: [{ language: "amxxpawn", value: constant.label }] };
...
export function doSignatures(
  ...
  while (result.identifier && attempts < MAX_OUTER_SEARCH) {
    ...
    const found = callables.find(
      (c) => c.identifier.toLowerCase() === result.identifier.toLowerCase(),
    );
  ...
  if (
    result.currentParamText &&
    result.currentParamText.trim().startsWith(".")
  ) {
...
function findIdentifierOccurrences(
  ...
  identifier: string,
  uri: string,
  locations: VSCLS.Location[],
  searchInStrings: boolean = false,
) {
  const lines = content.split(/\r?\n/);
...
export function getUsageTokens(
  ...
  for (const c of symbols.callables)
    symbolMap.set(c.identifier, { type: 0, modifier: 0 });
  ...
  for (const v of symbols.values)
    symbolMap.set(v.identifier, { type: 2, modifier: v.isConst ? 2 : 0 });
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    while ((tagMatch = tagRegex.exec(cleanLine)) !== null) {
      ...
      const existing = data.semanticTokens.find(
        (t) => t.line === lineIndex && t.char === char,
      );
    ...
    while ((match = regex.exec(cleanLine)) !== null) {
      ...
      const existing =
        data.semanticTokens.find(
          (t) => t.line === lineIndex && t.char === char,
        ) || tokens.find((t) => t.line === lineIndex && t.char === char);
      if (existing) continue;

      const localVars = data.localVariables.filter(
        (lv) => lv.scopeStartLine <= lineIndex && lv.scopeEndLine >= lineIndex,
      );
      ...
      if (localVar) {
        ...
        tokens.push({
          ...
          char,
          length: ident.length,
          tokenType: isParam ? 4 : 2,
          tokenModifiers: localVar.isConst ? 2 : 0,
        });
        continue;
      }
...
export function doPrepareRename(
  ...
  while ((match = regex.exec(line)) !== null) {
    if (
      ...
      position.character <= match.index + result.identifier.length
    ) {
      return {
        range: {
          start: { line: position.line, character: match.index },
            ...
            line: position.line,
            character: match.index + result.identifier.length,
          },
        },
...
export function doRename(
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    while ((match = lineRegex.exec(noStrings)) !== null) {
      edits.push(
        VSCLS.TextEdit.replace(
          {
            start: { line: lineIndex, character: match.index },
            end: {
              line: lineIndex,
              character: match.index + identifier.length,
            },
          },
          newName,
        ),
