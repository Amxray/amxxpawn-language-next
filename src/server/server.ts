...
let dependencyManager: DM.FileDependencyManager =
  new DM.FileDependencyManager();
    ...
    if (syncedSettings?.webApiLinks === true) {
      return data.resolvedInclusions.map((inc) => {
        ...
        return DocumentLink.create(
          range,
          `https://amxx-bg.info/api/${filename}`,
        );
      });
    }
...
function doReparse(document: TextDocument) {
  ...
  diagnostics.forEach((ds, uri) =>
    connection.sendDiagnostics({ uri: uri, diagnostics: ds }),
  );
...
connection.onDefinition(
  (params: TextDocumentPositionParams): Definition | null => {
    ...
    function inclusionLocation(
      inclusions: Types.ResolvedInclusion[],
    ): Location | null {
...
connection.onSignatureHelp(
  (params: TextDocumentPositionParams): SignatureHelp | null => {
    ...
    return Parser.doSignatures(
      document.getText(),
      params.position,
      Helpers.getSymbols(data, dependenciesData).callables,
    );
  },
);
...
connection.onDocumentSymbol((params): SymbolInformation[] | null => {
  ...
  return data.callables.map<SymbolInformation>((clb) => ({
    name: clb.identifier,
    location: {
      range: { start: clb.start, end: clb.end },
...
connection.onCompletion(
  (params: TextDocumentPositionParams): CompletionItem[] | null => {
    const document = documentsManager.get(params.textDocument.uri);
    if (!document) return null;
    const data = documentsData.get(document.uri);
    if (!data) return null;
...
documentsManager.onDidClose((event) => {
  ...
  if (docData) {
    Helpers.removeDependencies(
      docData.dependencies,
      dependencyManager,
      dependenciesData,
    );
    const allOpenDocsData = documentsManager
      .all()
      .map((doc) => documentsData.get(doc.uri))
      .filter((d): d is Types.DocumentData => d !== undefined);
    Helpers.removeUnreachableDependencies(
...
function parseFile(
  ...
  const oldDeps = data.dependencies.filter(
...
connection.languages.semanticTokens.on((params) => {
  ...
  const usageTokens = Parser.getUsageTokens(
    document.getText(),
    data,
    dependenciesData,
  );
